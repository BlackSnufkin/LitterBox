...
class RouteHelpers:
    ...
    def _load_pid_data(self, pid):
        ...
        result_folder = os.path.join(
            self.config["utils"]["result_folder"], f"dynamic_{pid}"
        )
    ...
    def _load_file_data(self, file_hash):
        ...
        result_path = self.utils.find_file_by_hash(
            file_hash, self.config["utils"]["result_folder"]
        )
        if not result_path:
        ...
        static_results = (
            self.utils.load_json_file(static_path)
            if os.path.exists(static_path)
            else None
        )
        dynamic_results = (
            self.utils.load_json_file(dynamic_path)
    ...
    def calculate_and_add_risk(self, data):
        ...
        if data["is_pid"]:
            risk_score, risk_factors = self.utils.calculate_risk(
                analysis_type="process", dynamic_results=data["dynamic_results"]
            )
        else:
            risk_score, risk_factors = self.utils.calculate_risk(
                analysis_type="file",
        ...
        risk_data = {"score": risk_score, "level": risk_level, "factors": risk_factors}
        ...
        for folder_type, folder_path in folders_to_clean.items():
            ...
            try:
                if folder_type == "uploads":
                    results["uploads_cleaned"] += self._clean_files_in_folder(
                        folder_path
                    )
                elif folder_type == "results":
                    results["result_cleaned"] += self._clean_folders_in_folder(
                        folder_path
                    )
                elif folder_type == "analysis":
                    results["analysis_cleaned"] += self._clean_process_folders(
                        folder_path
                    )
...
def register_routes(app):
    ...
    def upload_file():
        ...
        return (
            jsonify({"message": "File uploaded successfully", "file_info": file_info}),
            200,
        )
    ...
    def analyze_file(analysis_type, target):
        ...
        if is_pid:
            ...
            return _perform_file_analysis(
                analysis_type, target, analysis_manager, route_helpers, app
            )
    ...
    def _perform_pid_analysis(pid, analysis_manager, route_helpers, app):
        ...
        if not is_valid:
            app.logger.debug(
                f"PID validation failed for PID {pid}. Reason: {error_msg}"
            )
            return jsonify({"error": error_msg}), 404

        result_folder = os.path.join(
            route_helpers.config["utils"]["result_folder"], f"dynamic_{pid}"
        )
        ...
        return _handle_analysis_results(
            results, result_folder, "dynamic_analysis_results.json", route_helpers, app
        )
    ...
    def _perform_file_analysis(
        ...
        if analysis_type == "static" and target.isdigit():
            app.logger.debug(
                f"Static analysis requested on PID {target}. This is invalid."
            )
        ...
        file_path = route_helpers.utils.find_file_by_hash(
            target, app.config["utils"]["upload_folder"]
        )
        result_path = route_helpers.utils.find_file_by_hash(
        ...
        app.logger.debug(
            f"File found at: {file_path}, Results will be saved to: {result_path}"
        )
        ...
        if analysis_type == "static":
            ...
            cmd_args = _extract_and_validate_args(request, app.logger)
            app.logger.debug(
                f"Performing dynamic analysis on target: {file_path}, is_pid: False"
            )
            results = analysis_manager.run_dynamic_analysis(file_path, False, cmd_args)
            results_file = "dynamic_analysis_results.json"

        return _handle_analysis_results(
            results, result_path, results_file, route_helpers, app
        )
    ...
    def _handle_analysis_results(
        results, result_path, results_filename, route_helpers, app
    ):
                ...
                jsonify(
                    {
                        ...
                        "error": results.get("error", {}).get(
                            "message", "Process terminated early"
                        ),
                        "details": {
                            "termination_time": results.get("error", {}).get(
                                "termination_time"
                            ),
    ...
    def get_analysis_results(target, analysis_type):
        app.logger.debug(
            f"Received analysis results request for target: {target}, analysis_type: {analysis_type}"
        )
        ...
        risk_score, risk_level, risk_factors = route_helpers.calculate_and_add_risk(
            data
        )
        app.logger.debug(
            f"Calculated risk assessment - Score: {risk_score}, Level: {risk_level}"
        )
    ...
    def _render_byovd_results(data, route_helpers, app):
        ...
        if not _is_kernel_driver_file(filename):
            app.logger.warning(
                f"BYOVD analysis requested for non-driver file: {filename}"
            )
            return (
                render_template(
                    "error.html",
    ...
    def _render_file_info(data, utils, app):
        ...
        if "pe_info" in file_info:
            ...
            for section in pe_info["sections"]:
                section["entropy_risk"] = utils.get_entropy_risk_level(
                    section["entropy"]
                )
                app.logger.debug(
                    f"Calculated entropy risk for section {section.get('name', 'unknown')}: {section['entropy_risk']}"
                )
            ...
            app.logger.debug(
                f"Grouped suspicious imports for {len(grouped_imports)} DLLs"
            )

            if "checksum_info" in pe_info:
                checksum = pe_info["checksum_info"]
                checksum["stored_checksum"] = utils.format_hex(
                    checksum["stored_checksum"]
                )
                checksum["calculated_checksum"] = utils.format_hex(
                    checksum["calculated_checksum"]
                )
                app.logger.debug(
                    f"Formatted checksum values - Stored: {checksum['stored_checksum']}, Calculated: {checksum['calculated_checksum']}"
                )
    ...
    def _format_scan_duration(analysis_results, logger):
        try:
            raw_duration = analysis_results.get("analysis_metadata", {}).get(
                "total_duration", {}
            )
    ...
    def get_files_summary():
        ...
        try:
            ...
            app.logger.debug(
                f"Found {len(all_items)} items in results directory: {results_dir}"
            )
        except Exception as e:
        ...
        for item in all_items:
            ...
            if item.startswith("dynamic_"):
                _process_pid_summary(
                    item, item_path, pid_based_summary, utils, app.logger
                )
            else:
                _process_file_summary(
                    item, item_path, file_based_summary, utils, app.logger
                )
    ...
    def _process_pid_summary(item, item_path, pid_based_summary, utils, logger):
        ...
        try:
            ...
            hsb_detections = (
                dynamic_results.get("hsb", {}).get("findings", {}).get("detections", [])
            )
            ...
            pid_based_summary[pid] = {
                ...
                "analysis_summary": {
                    ...
                    "moneta": {
                        "total_findings": sum(
                            1
                            for key, value in moneta_findings.items()
                    ...
                    "hsb": {
                        "total_findings": sum(
                            ...
                            for det in hsb_detections
                            if det.get("pid") == int(pid)
                        ),
                        "findings": [
                            det for det in hsb_detections if det.get("pid") == int(pid)
                        ],
                    },
                },
            }
    ...
    def _process_file_summary(item, item_path, file_based_summary, utils, logger):
        ...
        try:
            ...
            file_based_summary[item] = {
                ...
                "detection_risk": file_info.get("entropy_analysis", {}).get(
    ...
    def doppelganger():
        ...
        analyzer = (
            BlenderAnalyzer(app.config, logger=app.logger)
            if analysis_type == "blender"
            else FuzzyHashAnalyzer(app.config, logger=app.logger)
        )

        if request.method == "GET":
            payload_hash = request.args.get("hash")
            if payload_hash:
                return _handle_doppelganger_hash_request(
                    analyzer, analysis_type, payload_hash, app
                )
    ...
    def _handle_doppelganger_hash_request(analyzer, analysis_type, payload_hash, app):
        if analysis_type == "blender":
            ...
            if (
                ...
                and comparison_result.get("status") == "error"
            ):
                return (
                    jsonify(
                        {"error": comparison_result.get("message", "Unknown error")}
                    ),
                    400,
                )
    ...
    def _handle_blender_initial_load(app):
        ...
        if os.path.exists(result_folder):
            files = [
                f for f in os.listdir(result_folder) if f.startswith("BlenderScan_")
            ]
            if files:
                latest_file = max(
                    files,
    ...
    def _handle_fuzzy_initial_load(analyzer):
        ...
        return render_template(
            "doppelganger.html", analysis_type="fuzzy", db_stats=db_stats
        )
    ...
    def holygrail():
        ...
        if request.method == "GET":
            ...
            if target_hash:
                ...
                try:
                    ...
                    file_path = utils.find_file_by_hash(
                        target_hash, app.config["utils"]["upload_folder"]
                    )
                    if not file_path:
                        app.logger.error(
                            f"Driver file not found for hash: {target_hash}"
                        )
                        return (
                            jsonify(
                                {"status": "error", "error": "Driver file not found"}
                            ),
                            404,
                        )
                    ...
                    result_path = utils.find_file_by_hash(
                        target_hash, app.config["utils"]["result_folder"]
                    )
                    if not result_path:
                        app.logger.error(
                            f"Result path not found for hash: {target_hash}"
                        )
                        return (
                            jsonify(
                                {"status": "error", "error": "Result path not found"}
                            ),
                            404,
                        )
                    ...
                    app.logger.debug(
                        f"Analysis completed with status: {results.get('status')}"
                    )
                    ...
                    if results["status"] == "completed":
                        ...
                        if compile_time:
                            ...
                            if (
                                "findings" in results
                                and "detailed_analysis" in results["findings"]
                            ):
                                results["findings"]["detailed_analysis"][
                                    "compile_time"
                                ] = compile_time

                        # Save results to results folder like other analysis types
                        route_helpers.save_analysis_results(
                            results, result_path, "byovd_results.json"
                        )
                        app.logger.debug(
                            f"HolyGrail results saved to: {result_path}/byovd_results.json"
                        )
                    ...
                    app.logger.error(
                        f"Exception during BYOVD analysis: {str(e)}", exc_info=True
                    )
                    return (
                        jsonify(
                            {
        ...
        if not _is_kernel_driver_file(file.filename):
            app.logger.debug(
                f"File '{file.filename}' is not a valid kernel driver format"
            )
    ...
    def cleanup():
        ...
        for folder_name in doppelganger_folders:
            ...
            if os.path.exists(folder_path):
                app.logger.debug(
                    f"Cleaning doppelganger folder contents: {folder_path}"
                )
                try:
        ...
        if os.path.exists(analysis_path):
            try:
                results["analysis_cleaned"] += route_helpers._clean_process_folders(
                    analysis_path
                )
            except Exception as e:
        ...
        if holygrail_config.get("enabled", False):
            ...
            if holygrail_results_path and os.path.exists(holygrail_results_path):
                app.logger.debug(
                    f"Cleaning HolyGrail results folder: {holygrail_results_path}"
                )
                try:
                    ...
                    for f in files:
                        ...
                        if os.path.isfile(file_path):
                            ...
                            results["analysis_cleaned"] += 1
                            app.logger.debug(
                                f"Deleted HolyGrail result folder: {file_path}"
                            )
                except Exception as e:
                    app.logger.error(
                    ...
                    results["errors"].append(
                        f"Error cleaning HolyGrail results: {str(e)}"
                    )
        ...
        return jsonify({"status": status, "message": message, "details": results}), (
        ...
        return jsonify(
            {
                ...
                "upload_folder_accessible": (
                    os.path.isdir(upload_folder) if upload_folder else False
                ),
                "issues": issues,
                "configuration": {
    ...
    def _check_holygrail_tool(holygrail_config, issues, logger):
        ...
        if holygrail_config.get("enabled", False):
            ...
            if not policies_path:
                ...
                issues.append(
                    f"HolyGrail: policies directory not found at {policies_path}"
                )
    ...
    def delete_file(target):
        ...
        upload_path = utils.find_file_by_hash(
            target, app.config["utils"]["upload_folder"]
        )
        result_path = utils.find_file_by_hash(
            target, app.config["utils"]["result_folder"]
    ...
    def api_static_results(target):
        ...
        if not os.path.exists(static_path):
            app.logger.warning(
                f"Static analysis results not found for target: {target}"
            )
    ...
    def api_dynamic_results(target):
        ...
        if target.isdigit():
            result_folder = os.path.join(
                app.config["utils"]["result_folder"], f"dynamic_{target}"
            )
            dynamic_path = os.path.join(result_folder, "dynamic_analysis_results.json")
        else:
            result_path = utils.find_file_by_hash(
        ...
        if not os.path.exists(dynamic_path):
            app.logger.warning(
                f"Dynamic analysis results not found for target: {target}"
            )
    ...
    @app.route("/api/results/<target>/risk", methods=["GET"])
    @error_handler
    ...
    def page_not_found(error):
        ...
        return (
            render_template("error.html", error=f"Page not found: {request.path}"),
            404,
        )
